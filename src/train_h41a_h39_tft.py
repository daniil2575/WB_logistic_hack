...
EXPERIMENT_NAME = "H41a_save_honest_oof_h39_tft_lite"
LOOKBACK = 24
N_DAYS = 21
BATCH_SIZE = 256
HIDDEN = 64
...
def main() -> None:
    ...
    cap = float(train_h[TARGET_COL].quantile(TARGET_Q))
    train_h["target_win"] = np.minimum(np.maximum(train_h[TARGET_COL].values, 0.0), cap).astype(np.float32)

    past_features = ["target_win", "hour_sin", "hour_cos", "dow_sin", "dow_cos"]
    future_features = ["hour_sin", "hour_cos", "dow_sin", "dow_cos"]

    means = train_h[past_features].mean()
    stds = train_h[past_features].std().replace(0, 1.0)
    train_h[past_features] = ((train_h[past_features] - means) / stds).astype(np.float32)
    test_h[future_features] = ((test_h[future_features] - means[future_features]) / stds[future_features]).astype(np.float32)

    target_mean = float(means["target_win"])
    target_std = float(stds["target_win"])
    ...
    for route_id, grp in train_h.groupby(ROUTE_COL):
        ...
        x_past = grp[past_features].values.astype(np.float32)
        y_train = grp["target_win"].values.astype(np.float32)
        y_true_raw = grp[TARGET_COL].values.astype(np.float32)
        ...
        for start in range(max_start):
            hist = x_past[start : start + LOOKBACK]
            future_slice = grp.iloc[start + LOOKBACK : start + LOOKBACK + PRED_LEN]
            fut = future_slice[future_features].values.astype(np.float32)
            horizon_idx = np.arange(1, PRED_LEN + 1, dtype=np.float32).reshape(-1, 1) / PRED_LEN
            base_ts = grp.loc[start + LOOKBACK - 1, TIME_COL]

            sample = {
                "x_hist": hist,
                "x_fut": np.hstack([fut, horizon_idx]).astype(np.float32),
                "y": y_train[start + LOOKBACK : start + LOOKBACK + PRED_LEN],
            ...
            if start >= max_start - 10:
                valid_samples.append(sample)
            else:
                train_samples.append(sample)

    if not train_samples or not valid_samples:
        raise RuntimeError("Not enough train/valid samples to run H41a.")

    print("Train samples:", len(train_samples))
    print("Valid samples:", len(valid_samples))
    print("Target cap:", cap)

    train_loader = DataLoader(TFTLiteDataset(train_samples), batch_size=BATCH_SIZE, shuffle=True, num_workers=0)
    valid_loader = DataLoader(TFTLiteDataset(valid_samples), batch_size=BATCH_SIZE, shuffle=False, num_workers=0)

    model = TFTLite(
        hist_dim=len(past_features),
        fut_dim=len(future_features) + 1,
    ...
    for epoch in range(1, EPOCHS + 1):
        tr_loss = run_epoch_tft(model, optimizer, criterion, train_loader, args.device, train_mode=True)
        va_loss = run_epoch_tft(model, optimizer, criterion, valid_loader, args.device, train_mode=False)
    ...
    with torch.no_grad():
        for batch in valid_loader:
            pred = model(
                batch["x_hist"].to(args.device),
                batch["x_fut"].to(args.device),
                batch["route_idx"].to(args.device),
                batch["office_idx"].to(args.device),
            ).cpu().numpy()
            valid_preds.append(pred)
            valid_true.append(batch["y_true_raw"].numpy())
    ...
    base_score, base_wape, base_bias = metric_parts(valid_true.reshape(-1), valid_preds_denorm.reshape(-1))
    best_k, cal_score, cal_wape, cal_bias = choose_global_k(valid_true.reshape(-1), valid_preds_denorm.reshape(-1))
    pred_final = np.maximum(0.0, valid_preds_denorm * best_k)
            ...
            oof_rows.append(
                {
                    ...
                    "y_pred": float(pred_final[i][h - 1]),
                    "model_name": EXPERIMENT_NAME,
                }
            )

    oof_df = pd.DataFrame(oof_rows)
    oof_name = outdir / "oof_h41a_h39_tft_lite.csv"
    ...
    for route_id, grp in train_h.groupby(ROUTE_COL):
        ...
        hist = grp[past_features].values.astype(np.float32)[-LOOKBACK:]
        test_route = test_h[test_h[ROUTE_COL] == route_id].sort_values(TIME_COL).copy()
        fut = test_route[future_features].values.astype(np.float32)
        horizon_idx = np.arange(1, PRED_LEN + 1, dtype=np.float32).reshape(-1, 1) / PRED_LEN
        test_rows.append(
            {
                "route_id": int(route_id),
    ...
    test_loader = DataLoader(TFTLiteDataset(test_rows), batch_size=BATCH_SIZE, shuffle=False, num_workers=0)
    ...
    submission = pd.DataFrame({
        ID_COL: test_sorted[ID_COL].values,
        "y_pred": preds,
    }).sort_values(ID_COL).reset_index(drop=True)
    sub_name = outdir / "submission_h41a_h39_tft_lite.csv"
    submission.to_csv(sub_name, index=False)

    ckpt_name = outdir / "h41a_h39_tft_lite.pt"
    torch.save(
        {
    ...
    save_json(outdir / "summary_h41a_h39_tft_lite.json", summary)
