...
EXPERIMENT_NAME = "H28_save_honest_oof_h27b_gru_winsorized_target"
LOOKBACK = 24
N_DAYS = 14
FRIDAY_WEIGHT = 1.0
BATCH_SIZE = 256
...
def main() -> None:
    ...
    target_cap = float(train_h[TARGET_COL].quantile(TARGET_Q))
    train_h["target_win"] = np.minimum(np.maximum(train_h[TARGET_COL].values, 0.0), target_cap).astype(np.float32)

    train_seq_features = ["target_win", "hour_sin", "hour_cos", "dow_sin", "dow_cos"]
    ...
    target_mean = float(feature_means["target_win"])
    target_std = float(feature_stds["target_win"])
    ...
    for route_id, grp in train_h.groupby(ROUTE_COL):
        ...
        y_win = grp["target_win"].values.astype(np.float32)
            ...
            local_samples.append(
                {
                    ...
                    "y": y_win[start + LOOKBACK : start + LOOKBACK + PRED_LEN],
    ...
    if not train_samples or not valid_samples:
        raise RuntimeError("Not enough train/valid samples to run H28.")

    print("Train samples:", len(train_samples))
    print("Valid samples:", len(valid_samples))
    print("Target cap:", target_cap)
    ...
    oof_name = outdir / f"oof_h28_h27b_gru_winsorized_target{args.suffix}.csv"
    ...
    sub_name = outdir / f"submission_h28_h27b_gru_winsorized_target{args.suffix}.csv"
    submission.to_csv(sub_name, index=False)

    ckpt_name = outdir / "h28_h27b_gru_winsorized_target.pt"
    torch.save(
        {
    ...
    save_json(outdir / "summary_h28_h27b_gru_winsorized_target.json", summary)
